            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left text-sm border-collapse">
                    <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
                        <tr>
                            <SortableHeader<Zona> label="Código" sortKey="codigo" sortConfig={sortConfig} onSort={requestSort} className="w-24" />
                            <SortableHeader<Zona> label="Nombre de Zona" sortKey="nombre" sortConfig={sortConfig} onSort={requestSort} />
                            <th className="px-4 py-3 font-semibold text-slate-900 dark:text-slate-100">Descripción</th>
                            <th className="px-4 py-3 font-semibold text-slate-900 dark:text-slate-100 text-right">Acciones</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-800 bg-white dark:bg-slate-950">
                        {items.map((zone) => (
                            <tr key={zone.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                                ...
                                <td className="px-4 py-3">
                                    <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-1">
                                        {zone.descripcion || <span className="italic opacity-50 text-[10px]">Sin descripción</span>}
                                    </p>
                                </td>
                                <td className="px-4 py-3 text-right">
                                    <div className="flex items-center justify-end gap-2">
                                        <button onClick={() => onEdit(zone)} className="p-1.5 text-slate-400 hover:text-brand transition-colors rounded hover:bg-slate-100 dark:hover:bg-slate-800" title="Editar">
                                            <span className="material-symbols-outlined text-[18px]">edit</span>
                                        </button>
                                        <button onClick={() => onDelete(zone)} className="p-1.5 text-slate-400 hover:text-red-500 transition-colors rounded hover:bg-red-50 dark:hover:bg-red-900/10" title="Eliminar">
