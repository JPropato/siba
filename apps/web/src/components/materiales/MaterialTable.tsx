            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left text-sm border-collapse">
                    <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
                        <tr>
                            <th className="px-4 py-3 font-semibold text-slate-900 dark:text-slate-100">SKU / Código</th>
                            <th className="px-4 py-3 font-semibold text-slate-900 dark:text-slate-100">Material</th>
                            <th className="px-4 py-3 font-semibold text-slate-900 dark:text-slate-100">Categoría</th>
                            <th className="px-4 py-3 font-semibold text-slate-900 dark:text-slate-100">Presentación</th>
                            <th className="px-4 py-3 font-semibold text-slate-900 dark:text-slate-100 text-right">Costo</th>
                            <th className="px-4 py-3 font-semibold text-slate-900 dark:text-slate-100 text-center">Margen</th>
                            <th className="px-4 py-3 font-semibold text-slate-900 dark:text-slate-100 text-right">Precio Venta</th>
                                    ...
                                    <div className="flex flex-col text-xs">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            {m.stockMinimo !== null && m.stockMinimo > 0 && (
                                                <span className="text-[10px] text-amber-500 font-bold" title={`Stock Mínimo: ${m.stockMinimo}`}>
                                                    (Min: {m.stockMinimo})
                                                </span>
                                            )}
                                        </div>
