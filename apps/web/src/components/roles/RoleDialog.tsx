        ...
        >
            <form id="role-form" onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                ...
                <div className="space-y-3">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Permisos</label>
                    {isEditingSuperAdmin && (
                    ...
                    <div className="space-y-3 max-h-[40vh] overflow-y-auto pr-2">
                        {Object.entries(permissionsGrouped.grouped).map(([modulo, perms]) => {
                            ...
                            return (
                                <div key={modulo} className="border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden">
                                    <div
                                    ...
                                    <div className="px-4 py-3 space-y-2">
                                        {perms.map((perm) => (
                                            <label key={perm.id} className="flex items-start gap-3 cursor-pointer group">
                                                ...
                                                />
                                                <div>
                                                    <p className="text-sm font-medium text-slate-900 dark:text-white group-hover:text-brand">{perm.codigo}</p>
                                                    {perm.descripcion && <p className="text-xs text-slate-400">{perm.descripcion}</p>}
                                                </div>
                                            </label>
                                        ))}
                                    </div>
