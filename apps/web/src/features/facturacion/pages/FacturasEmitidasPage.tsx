...
import type { FacturaEmitida, EstadoFacturaEmitida, CreateFacturaEmitidaDto } from '../types';
import { ESTADO_FACTURA_EMITIDA_CONFIG } from '../types';
import { TIPO_COMPROBANTE_LABELS } from '../../compras/types';
import {
  useFacturasEmitidas,
  useCreateFacturaEmitida,
  useUpdateFacturaEmitida,
  useAnularFacturaEmitida,
  useFacturaEmitida,
  useRegistrarCobro,
...
const ESTADO_OPTIONS = Object.entries(ESTADO_FACTURA_EMITIDA_CONFIG).map(([value, cfg]) => ({
  value,
  label: cfg.label,
}));
...
export default function FacturasEmitidasPage() {
  ...
  // Cobro dialog
  const [cobroFactura, setCobroFactura] = useState<FacturaEmitida | null>(null);
  const [cobroMonto, setCobroMonto] = useState('');
  const [cobroFecha, setCobroFecha] = useState('');
  const [cobroMedioPago, setCobroMedioPago] = useState<string>('TRANSFERENCIA');
  const [cobroCuentaId, setCobroCuentaId] = useState('');
  const [cobroObservaciones, setCobroObservaciones] = useState('');
  // Cheque inline fields (auto-create)
  const [chequeNumero, setChequeNumero] = useState('');
  const [chequeTipo, setChequeTipo] = useState<string>('FISICO');
  ...
  // Queries
  const { data, isLoading, refetch } = useFacturasEmitidas({
    search: debouncedSearch || undefined,
    clienteId: filterCliente ? Number(filterCliente) : undefined,
    estado: (filterEstado as EstadoFacturaEmitida) || undefined,
    page,
    limit: 20,
  });
  ...
  // Detail drawer query
  const { data: facturaDetalle } = useFacturaEmitida(detalleFacturaId);

  // Mutations
  const createMutation = useCreateFacturaEmitida();
  const updateMutation = useUpdateFacturaEmitida();
  const anularMutation = useAnularFacturaEmitida();
  const registrarCobroMutation = useRegistrarCobro();
  ...
  const activeFiltersCount = (search ? 1 : 0) + (filterCliente ? 1 : 0) + (filterEstado ? 1 : 0);

  // --- Form handlers (moved up) ---

  const handleEdit = (fac: FacturaEmitida) => {
    setEditingFactura(fac);
    setFormClienteId(String(fac.clienteId));
    ...
    setFormTicketId(fac.ticketId ? String(fac.ticketId) : '');
    setShowClasificacion(
      !!fac.cuentaContableId || !!fac.centroCostoId || !!fac.obraId || !!fac.ticketId
    );
    setIsDrawerOpen(true);
  };

  const handleAnular = async (fac: FacturaEmitida) => {
    const ok = await confirm({
      title: 'Anular factura emitida',
      message: `¿Anular la factura ${TIPO_COMPROBANTE_LABELS[fac.tipoComprobante as keyof typeof TIPO_COMPROBANTE_LABELS]} ${padPV(fac.puntoVenta)}-${fac.numeroComprobante}? Esta accion no se puede deshacer.`,
      confirmLabel: 'Anular',
      variant: 'danger',
    });
    if (!ok) return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formClienteId || !formTipo || !formPuntoVenta || !formNumero || !formFechaEmision) {
    ...
    const dto: CreateFacturaEmitidaDto = {
      ...
      ticketId: formTicketId ? Number(formTicketId) : null,
    };
    ...
    try {
      if (editingFactura) {
        ...
        toast.success('Factura emitida creada');
      }
  ...
  // --- Cobro handlers ---
  const showChequeFields = cobroMedioPago === 'CHEQUE' || cobroMedioPago === 'ECHEQ';

  const handleOpenCobro = (fac: FacturaEmitida) => {
    setCobroFactura(fac);
    setCobroMonto(String(fac.saldoPendiente));
    setCobroFecha(new Date().toISOString().slice(0, 10));
    setCobroMedioPago('TRANSFERENCIA');
    setCobroCuentaId('');
  ...
  const handleSubmitCobro = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!cobroFactura || !cobroCuentaId || !cobroMonto || !cobroFecha) {
      toast.error('Complete los campos obligatorios');
      return;
    }
    const cobroMontoNum = numVal(cobroMonto);
    if (cobroMontoNum <= 0) {
      toast.error('El monto debe ser mayor a 0');
      return;
    }
    if (cobroMontoNum > cobroFactura.saldoPendiente + 0.01) {
      toast.error(`El monto no puede superar el saldo pendiente ($${cobroFactura.saldoPendiente})`);
      return;
    }

    // Validate cheque fields if cheque payment
    if (
      showChequeFields &&
      (!chequeNumero || !chequeBanco || !chequeFechaEmision || !chequeFechaCobro || !chequeMonto)
    ) {
    ...
    try {
      await registrarCobroMutation.mutateAsync({
        facturaId: cobroFactura.id,
        monto: numVal(cobroMonto),
        fechaCobro: cobroFecha,
        medioPago: cobroMedioPago,
        cuentaId: Number(cobroCuentaId),
        chequeData: showChequeFields
          ? {
              numero: chequeNumero,
              tipo: chequeTipo as 'FISICO' | 'ECHEQ',
              bancoEmisor: chequeBanco,
              fechaEmision: chequeFechaEmision,
              fechaCobro: chequeFechaCobro,
              monto: numVal(chequeMonto),
              emisor: chequeEmisor || null,
            }
          : null,
        observaciones: cobroObservaciones || null,
      });
      toast.success('Cobro registrado. Se creo movimiento de ingreso.');
      ...
      <div className="px-4 pt-3 pb-6 sm:px-6 space-y-5 animate-in fade-in duration-500">
        ...
        <CollapsibleFilters activeFiltersCount={activeFiltersCount}>
          <div className="flex flex-wrap gap-3">
            ...
            <div className="w-48">
              <Select
                options={[{ value: '', label: 'Todos los clientes' }, ...clientesOptions]}
                value={filterCliente}
                onChange={(v) => {
                  setFilterCliente(v);
                  setPage(1);
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            description="Emita facturas a sus clientes para llevar el control de facturacion."
          />
        ) : (
          <div className="w-full overflow-hidden rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm border-collapse">
                ...
                <tbody>
                  {facturas.map((fac) => {
                    const estadoCfg = ESTADO_FACTURA_EMITIDA_CONFIG[fac.estado];
                    const tipoLabel =
                      TIPO_COMPROBANTE_LABELS[
                        fac.tipoComprobante as keyof typeof TIPO_COMPROBANTE_LABELS
                      ] ?? fac.tipoComprobante;
                    return (
                      <tr
                        key={fac.id}
                      ...
                      >
                        <td className="px-3 py-2.5">
                          <div>
                            <span className="text-sm font-medium text-slate-900 dark:text-white">
                              {tipoLabel} {padPV(fac.puntoVenta)}-{fac.numeroComprobante}
                            </span>
                            <span className="block text-[10px] text-slate-400 md:hidden">
                              {fac.cliente?.razonSocial}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="px-2 py-2.5 text-right" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center justify-end gap-1">
                            {(fac.estado === 'PENDIENTE' || fac.estado === 'COBRO_PARCIAL') && (
                              <button
                                onClick={() => handleOpenCobro(fac)}
        ...
        <DialogBase
          ...
          type="drawer"
          maxWidth="md"
          title={editingFactura ? 'Editar Factura' : 'Nueva Factura Emitida'}
          icon={
        ...
        >
          <form id="factura-emitida-form" onSubmit={handleSubmit} className="space-y-4">
            <Select
              label="Cliente *"
        ...
        <DialogBase
          ...
          footer={
            <>
              <Button variant="ghost" onClick={() => setCobroFactura(null)}>
                Cancelar
              </Button>
              <Button type="submit" form="cobro-form" isLoading={registrarCobroMutation.isPending}>
                Registrar Cobro
              </Button>
            </>
          }
        >
          {cobroFactura && (
            <form id="cobro-form" onSubmit={handleSubmitCobro} className="space-y-4">
              <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-lg text-sm space-y-1">
                <p className="font-medium text-slate-900 dark:text-white">
                  {
                    TIPO_COMPROBANTE_LABELS[
                      cobroFactura.tipoComprobante as keyof typeof TIPO_COMPROBANTE_LABELS
                    ]
                  }{' '}
                  {padPV(cobroFactura.puntoVenta)}-{cobroFactura.numeroComprobante}
                </p>
                <p className="text-slate-500">{cobroFactura.cliente?.razonSocial}</p>
                <p className="text-slate-500">
                  Saldo pendiente:{' '}
                  <span className="font-bold text-slate-900 dark:text-white">
                    {fmt(cobroFactura.saldoPendiente)}
                  </span>
                </p>
              </div>
              ...
              <Select
                ...
                onChange={(v) => {
                  setCobroMedioPago(v);
                  if (v === 'CHEQUE') setChequeTipo('FISICO');
                  if (v === 'ECHEQ') setChequeTipo('ECHEQ');
                }}
              />
              ...
              <Select
                ...
                value={cobroCuentaId}
                onChange={(v) => setCobroCuentaId(v)}
                placeholder="Seleccionar cuenta..."
              />

              {/* Cheque inline fields - auto-create when client pays with cheque */}
              {showChequeFields && (
                <div className="p-3 border border-slate-200 dark:border-slate-800 rounded-lg space-y-3">
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">
                    Datos del Cheque (se creara en Cartera)
                  </p>
        ...
        {/* Drawer detalle factura emitida */}
        <DialogBase
          isOpen={!!detalleFacturaId}
          onClose={() => setDetalleFacturaId(null)}
          type="drawer"
          maxWidth="md"
          title="Detalle de Factura Emitida"
          icon={
            <div className="p-2.5 rounded-xl bg-blue-100 dark:bg-blue-900/30 text-blue-600">
              <FileText className="h-5 w-5" />
            </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-lg font-bold text-slate-900 dark:text-white">
                    {
                      TIPO_COMPROBANTE_LABELS[
                        facturaDetalle.tipoComprobante as keyof typeof TIPO_COMPROBANTE_LABELS
                      ]
                    }{' '}
                    {padPV(facturaDetalle.puntoVenta)}-{facturaDetalle.numeroComprobante}
                  </span>
                  <span
                    className={`inline-flex px-2 py-0.5 text-[10px] font-bold rounded-full ${ESTADO_FACTURA_EMITIDA_CONFIG[facturaDetalle.estado].bgColor} ${ESTADO_FACTURA_EMITIDA_CONFIG[facturaDetalle.estado].color}`}
                  >
                    {ESTADO_FACTURA_EMITIDA_CONFIG[facturaDetalle.estado].label}
                  </span>
                </div>
                <p className="text-sm text-slate-500">
                  {facturaDetalle.cliente?.razonSocial}{' '}
                  {facturaDetalle.cliente?.cuit && `(${facturaDetalle.cliente.cuit})`}
                </p>
              ...
              <div className="border-t border-slate-200 dark:border-slate-800 pt-3 space-y-1.5">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                  ...
                  <span className="text-right font-mono text-green-600">
                    {fmt(facturaDetalle.montoCobrado)}
                  </span>
              ...
              {facturaDetalle.cae && (
                <div className="border-t border-slate-200 dark:border-slate-800 pt-3 space-y-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                    ...
                    {facturaDetalle.caeFechaVto && (
                      <>
                        <span className="text-slate-400">Vto. CAE:</span>
                        <span className="text-right text-slate-700 dark:text-slate-300">
                          {fmtDate(facturaDetalle.caeFechaVto)}
                        </span>
                      </>
                    )}
                  </div>
                  ...
                  <div className="space-y-1 text-sm">
                    ...
                    {facturaDetalle.ticket && (
                      <div className="flex justify-between">
                        ...
                        <span className="text-slate-700 dark:text-slate-300">
                          TKT-{String(facturaDetalle.ticket.codigoInterno).padStart(5, '0')}
                        </span>
                      </div>
                    )}
                  </div>
              ...
              <div className="border-t border-slate-200 dark:border-slate-800 pt-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  {(facturaDetalle.estado === 'PENDIENTE' ||
                    facturaDetalle.estado === 'COBRO_PARCIAL') && (
                    <button
                      onClick={() => {
                        setDetalleFacturaId(null);
                        handleOpenCobro(facturaDetalle);
                      }}
                ...
                {facturaDetalle.cobros && facturaDetalle.cobros.length > 0 ? (
                  <div className="space-y-2">
                    {facturaDetalle.cobros.map((cobro) => (
                      <div
                        key={cobro.id}
                      ...
                      >
                        <div>
                          <span className="text-slate-700 dark:text-slate-300">
                            {fmtDate(cobro.fechaCobro)}
                          </span>
                          <span className="mx-2 text-slate-300">|</span>
                          <span className="text-slate-500">
                            {MEDIO_PAGO_LABELS[cobro.medioPago as MedioPago] ?? cobro.medioPago}
                          </span>
                          {cobro.cheque && (
                            <>
                              <span className="mx-2 text-slate-300">|</span>
                              <span className="text-slate-500">Cheque {cobro.cheque.numero}</span>
                            </>
                          )}
                          {cobro.movimiento && (
                            <>
                  ...
                  <p className="text-sm text-slate-400 italic">Sin cobros registrados</p>
                )}
              </div>
            </div>
          ) : detalleFacturaId ? (
