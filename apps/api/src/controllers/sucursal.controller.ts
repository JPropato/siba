...
export const create = async (req: Request, res: Response) => {
    try {
        ...
        if (!cliente) return res.status(400).json({ error: 'El cliente seleccionado no es válido o no existe.' });
        ...
        if (!zona) return res.status(400).json({ error: 'La zona seleccionada no es válida o no existe.' });
        ...
        res.status(500).json({ error: 'Error al crear sede. ' + (error instanceof Error ? error.message : '') });
    }
};
