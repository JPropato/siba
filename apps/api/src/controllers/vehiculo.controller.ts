...
const createVehiculoSchema = z.object({
    ...
    anio: z.number().int().min(1900).max(new Date().getFullYear() + 1).optional().nullable(),
    tipo: z.string().optional().nullable(),
    zonaId: z.number().int().optional().nullable(),
    proximosKm: z.number().int().min(0).optional().nullable(),
    proximoService: z.string().datetime().optional().nullable().or(z.date().optional().nullable()),
...
export const create = async (req: Request, res: Response) => {
    try {
        ...
        if (body.zonaId) {
            ...
            if (!zona) return res.status(400).json({ error: 'La zona seleccionada no existe o no es válida.' });
        }
...
export const update = async (req: Request, res: Response) => {
    try {
        ...
        if (body.patente && body.patente !== vehiculo.patente) {
            ...
            if (existing) return res.status(400).json({ error: 'Ya existe otro vehículo con esa patente.' });
        }
        ...
        const updated = await prisma.vehiculo.update({
            ...
            data: {
                ...
                proximoService: body.proximoService ? new Date(body.proximoService as string) : (body.proximoService === null ? null : undefined)
            },
            include: {
                zona: { select: { nombre: true } }
            }
