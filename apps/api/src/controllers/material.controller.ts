...
export const create = async (req: Request, res: Response) => {
    try {
        ...
        if (existing) return res.status(400).json({ error: 'Ya existe un material con ese código de artículo.' });
...
export const update = async (req: Request, res: Response) => {
    try {
        ...
        if (body.codigoArticulo && body.codigoArticulo !== currentMaterial.codigoArticulo) {
            ...
            if (existing) return res.status(400).json({ error: 'Ya existe otro material con ese código de artículo.' });
        }
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const precioCostoChanged = body.precioCosto !== undefined && Number(body.precioCosto) !== Number(currentMaterial.precioCosto);
            const precioVentaChanged = body.precioVenta !== undefined && Number(body.precioVenta) !== Number(currentMaterial.precioVenta);

            if (precioCostoChanged || precioVentaChanged) {
                await tx.historialPrecio.create({
                    data: {
                        materialId: id,
                        precioCosto: body.precioCosto !== undefined ? body.precioCosto : Number(currentMaterial.precioCosto),
                        precioVenta: body.precioVenta !== undefined ? body.precioVenta : Number(currentMaterial.precioVenta),
                        porcentajeRentabilidad: body.porcentajeRentabilidad !== undefined ? body.porcentajeRentabilidad : Number(currentMaterial.porcentajeRentabilidad)
                    }
                });
            }
