...
export const create = async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({ error: 'Error al crear zona. ' + (error instanceof Error ? error.message : '') });
    }
};
    ...
    try {
        ...
        if (hasSedes) {
            return res.status(400).json({
                error: 'No se puede eliminar la zona porque tiene sedes asociadas. Por favor, mueva o elimine las sedes primero.'
            });
        }
