...
const createClientSchema = z.object({
    ...
    cuit: z.string().optional().nullable().transform(val => val === '' ? null : val),
...
export const create = async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({ error: 'Error al crear cliente', details: error instanceof Error ? error.message : String(error) });
    }
};
...
export const update = async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({ error: 'Error al actualizar cliente', details: error instanceof Error ? error.message : String(error) });
    }
};
    ...
    try {
        ...
        if (hasSedes) {
            return res.status(400).json({
                error: 'No se puede eliminar el cliente porque tiene sedes (sucursales) asociadas. Por favor, elimine las sedes primero.'
            });
        }
